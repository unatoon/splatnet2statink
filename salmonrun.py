...
def salmon_load_json():
    ...
    results_list = requests.get(
        url, headers=app_head, cookies=dict(iksm_session=cookie)
    )
...
def set_teammates(payload, job_id):
    ...
    try:
        ...
        print(
            "Problem retrieving shift details. Continuing without teammates' scoreboard statistics."
        )
    ...
    for num in range(num_teammates):
        ...
        # Special weapon
        payload["teammates"][num]["special"] = translate_specials[
            int(results["other_results"][num]["special"]["id"])
        ]

        # Rescues, deaths, egg stats
        payload["teammates"][num]["rescue"] = results["other_results"][num][
            "help_count"
        ]
        payload["teammates"][num]["death"] = results["other_results"][num]["dead_count"]
        payload["teammates"][num]["golden_egg_delivered"] = results["other_results"][
            num
        ]["golden_ikura_num"]
        payload["teammates"][num]["power_egg_collected"] = results["other_results"][
            num
        ]["ikura_num"]

        # Special uses, main weapon
        weapon_list = results["other_results"][num]["weapon_list"]
        payload["teammates"][num]["special_uses"] = results["other_results"][num][
            "special_counts"
        ]  # list
        payload["teammates"][num]["weapons"] = [
            dbs.weapons.get(int(d["id"]), None) for d in weapon_list
        ]

        # Boss kills
        boss_kills = {}
        boss_kills["goldie"] = results["other_results"][num]["boss_kill_counts"]["3"][
            "count"
        ]
        boss_kills["steelhead"] = results["other_results"][num]["boss_kill_counts"][
            "6"
        ]["count"]
        boss_kills["flyfish"] = results["other_results"][num]["boss_kill_counts"]["9"][
            "count"
        ]
        boss_kills["scrapper"] = results["other_results"][num]["boss_kill_counts"][
            "12"
        ]["count"]
        boss_kills["steel_eel"] = results["other_results"][num]["boss_kill_counts"][
            "13"
        ]["count"]
        boss_kills["stinger"] = results["other_results"][num]["boss_kill_counts"]["14"][
            "count"
        ]
        boss_kills["maws"] = results["other_results"][num]["boss_kill_counts"]["15"][
            "count"
        ]
        boss_kills["griller"] = results["other_results"][num]["boss_kill_counts"]["16"][
            "count"
        ]
        boss_kills["drizzler"] = results["other_results"][num]["boss_kill_counts"][
            "21"
        ]["count"]
...
def salmon_post_shift(i, results):
    ...
    namespace = uuid.UUID("{418fe150-cb33-11e8-8816-d050998473ba}")
    ...
    for wave in range(num_waves):
        ...
        event = (
            results[i]["wave_details"][wave]["event_type"]["key"]
            .replace("the-", "", 1)
            .replace("-", "_")
        )
        ...
        # Water level
        payload["waves"][wave]["water_level"] = results[i]["wave_details"][wave][
            "water_level"
        ][
            "key"
        ]  # low, normal, high
        ...
        # Eggs
        payload["waves"][wave]["golden_egg_quota"] = results[i]["wave_details"][wave][
            "quota_num"
        ]
        payload["waves"][wave]["golden_egg_appearances"] = results[i]["wave_details"][
            wave
        ]["golden_ikura_pop_num"]
        payload["waves"][wave]["golden_egg_delivered"] = results[i]["wave_details"][
            wave
        ]["golden_ikura_num"]
        payload["waves"][wave]["power_egg_collected"] = results[i]["wave_details"][
            wave
        ]["ikura_num"]
    ...
    # Special weapon
    translate_specials = {2: "pitcher", 7: "presser", 8: "jetpack", 9: "chakuchi"}
    payload["my_data"]["special"] = translate_specials[
        int(results[i]["my_result"]["special"]["id"])
    ]

    # Rescues, deaths, egg stats
    payload["my_data"]["rescue"] = results[i]["my_result"]["help_count"]
    payload["my_data"]["death"] = results[i]["my_result"]["dead_count"]
    payload["my_data"]["golden_egg_delivered"] = results[i]["my_result"][
        "golden_ikura_num"
    ]
    payload["my_data"]["power_egg_collected"] = results[i]["my_result"]["ikura_num"]

    # Species, gender
    payload["my_data"]["species"] = results[i]["player_type"]["species"][
        :-1
    ]  # inklings -> inkling
    ...
    payload["my_data"]["special_uses"] = results[i]["my_result"][
        "special_counts"
    ]  # list
    payload["my_data"]["weapons"] = [
        dbs.weapons.get(int(d["id"]), None) for d in weapon_list
    ]
    ...
    postshift = requests.post(
        url, headers=auth, data=json.dumps(payload), allow_redirects=False
    )
...
def salmon_get_data():
    ...
    if cookie == "" or "code" in data:
        print(
            "Blank or invalid cookie. Please run splatnet2statink in non-Salmon Run mode to obtain a cookie."
        )
        exit(1)
...
def salmon_get_num_shifts(results):
    ...
    if len(results) == 0:
        ...
        print(
            "You do not have {} Salmon Run shifts recorded on SplatNet 2. Uploading all {}.".format(
                n, len(results)
            )
        )
        n = len(results)
...
def upload_salmon_run(s2s_version, s2s_cookie, s2s_api_key, s2s_app_head, r_flag):
    ...
    if r_flag:  # upload all unuploaded shifts
        ...
        new_results = list(
            filter(lambda r: not (r["job_id"] in statink_shifts), results)
        )
