...
def populate_battles(s_flag, t_flag, r_flag, debug):
    ...
    if r_flag:
        ...
        try:
            statink_battles = json.loads(
                resp.text
            )  # 100 recent battles on stat.ink. should avoid dupes
        except:
            print(
                "Encountered an error while checking recently-uploaded battles. Is stat.ink down?"
            )
            sys.exit(1)
    ...
    for i, result in reversed(list(enumerate(results))):
        ...
        battles.append(
            bn
        )  # for main process, don't upload any of the ones already in the file
        if r_flag:
            if (
                bn not in statink_battles
            ):  # one of the splatnet battles isn't on stat.ink (unuploaded)
                if not printed:
                    printed = True
...
def monitor_battles(s_flag, t_flag, r_flag, secs, debug):
    ...
    results = load_results(
        "monitor"
    )  # make sure we can do it first. if error, throw it before main process

    battles = populate_battles(s_flag, t_flag, r_flag, debug)
    wins, losses, splatfest_wins, splatfest_losses, mirror_matches = [
        0
    ] * 5  # init all to 0
    ...
    try:
        while True:
            ...
            for i, result in reversed(
                list(enumerate(results))
            ):  # reversed chrono order
                if int(result["battle_number"]) not in battles:
                    if result["game_mode"]["key"] == "private" and custom_key_exists(
                        "ignore_private", True
                    ):
                        pass
                    else:
                        ...
                        splatfest_match = (
                            True
                            if result["game_mode"]["key"] in ["fes_solo", "fes_team"]
                            else False
                        )
                        ...
                        mapname = translate_stages.get(
                            translate_stages.get(int(result["stage"]["id"]), ""),
                            fullname,
                        )
                        print(
                            "New battle result detected at {}! ({}, {})".format(
        ...
        for i, result in reversed(list(enumerate(results))):
            if int(result["battle_number"]) not in battles:
                if result["game_mode"]["key"] == "private" and custom_key_exists(
                    ...
                    fullname = result["stage"]["name"]
                    mapname = translate_stages.get(
                        translate_stages.get(int(result["stage"]["id"]), ""), fullname
                    )
                    print(
                        "New battle result detected at {}! ({}, {})".format(
                            datetime.datetime.fromtimestamp(int(result["start_time"]))
                            .strftime("%I:%M:%S %p")
                            .lstrip("0"),
                            mapname,
                            worl,
        ...
        print(
            "%d win%s and %d loss%s this session." % (wins, w_plural, losses, l_plural)
        )
        if splatfest_wins != 0 or splatfest_losses != 0:
            ...
            print(
                "{} win{} and {} loss{} against the other Splatfest team.".format(
                    splatfest_wins, w_plural, splatfest_losses, l_plural
                )
            )
            print(
                "{} mirror match{} against your Splatfest team.".format(
                    mirror_matches, m_plural
                )
            )
        print("Bye!")
...
def post_battle(i, results, s_flag, t_flag, m_flag, sendgears, debug, ismonitor=False):
    ...
    ver4 = (
        True if "version" in results[i] and results[i]["version"] >= 4 else False
    )  # splatfest only
    ...
    k_or_a = (
        results[i]["player_result"]["kill_count"]
        + results[i]["player_result"]["assist_count"]
    )
    ...
    try:  # udemae not present in all modes
        rank_after = results[i]["udemae"][
            "name"
        ].lower()  # non-null after playing first solo battle
    ...
    try:
        if results[i]["udemae"][
            "is_x"
        ]:  # == true. results[i]["udemae"]["number"] should be 128
            payload["x_power_after"] = results[i][
                "x_power"
            ]  # can be null if not played placement games
            if mode == "gachi":
                payload["estimate_x_power"] = results[i][
                    "estimate_x_power"
                ]  # team power, approx
            payload["worldwide_rank"] = results[i][
                "rank"
    ...
    if mode == "league":
        ...
        payload["my_team_estimate_league_point"] = results[i][
            "my_estimate_league_point"
        ]
        payload["his_team_estimate_league_point"] = results[i][
    ...
    if mode == "fes":
        ...
        # SPECIAL CASE - CHAMPION (999) TO KING/QUEEN
            ...
            if title_before == title_after:  # within same title
                ...
                title_before == 0 and title_after == 1
            ):  # fanboy/girl (100) to fiend (250)
                fest_rank_rollover = 10 * multiplier
            elif (
                title_before == 1 and title_after == 2
            ):  # fiend (250) to defender (500)
                fest_rank_rollover = 25 * multiplier
            elif (
                title_before == 2 and title_after == 3
            ):  # defender (500) to champion (999)
    ...
    ################
    if (
        YOUR_COOKIE != "" or debug
    ):  # requires online (or battle json). if no cookie, don't do - will fail
        mystats = [
            mode,
            rule,
            result,
    ...
    if not debug:
            ...
            if image_result_url:
                ...
                if image_result.ok:
                    if not s_flag:  # normal scoreboard
                        payload["image_result"] = BytesIO(
                            image_result.content
                        ).getvalue()
                    else:
                        players = [
                            0
                        ] * 8  # in case battles are < 8 people. mark missing player-positions
                        my_missing = 4 - (
                            len(payload["splatnet_json"]["my_team_members"]) + 1
                        )
                        their_missing = 4 - len(
                            payload["splatnet_json"]["other_team_members"]
                        )
                        ...
                        for p in range(len(payload["players"])):
                            ...
                            try:
                                ...
                                is_player_me = None
                            lowest_zero = players.index(
                                0
                            )  # fill in 0s (uninits) with values
        ...
        if sendgears:  # if most recent
            ...
            if stage >= 100:  # fav_stage can't be Shifty Station(s)
                stages_ints = [
                    k for k in translate_stages.keys() if isinstance(k, int) and k < 100
                ]
            ...
            settings = {
                "stage": fav_stage,
                "color": translate_profile_color[random.randrange(0, 6)],
            }
            share_result = requests.post(
                url_profile,
                headers=app_head,
                cookies=dict(iksm_session=YOUR_COOKIE),
                data=settings,
            )
            if share_result.ok:
    ...
    for j in range(3):
        try:
            headgear_subs[j] = results[i]["player_result"]["player"]["head_skills"][
                "subs"
            ][j]["id"]
        except:
            headgear_subs[j] = "-1"
        try:
            clothing_subs[j] = results[i]["player_result"]["player"]["clothes_skills"][
                "subs"
            ][j]["id"]
        except:
            clothing_subs[j] = "-1"
        try:
            shoes_subs[j] = results[i]["player_result"]["player"]["shoes_skills"][
                "subs"
            ][j]["id"]
        except:
            shoes_subs[j] = "-1"
    headgear_main = results[i]["player_result"]["player"]["head_skills"]["main"]["id"]
    clothing_main = results[i]["player_result"]["player"]["clothes_skills"]["main"][
        "id"
    ]
    shoes_main = results[i]["player_result"]["player"]["shoes_skills"]["main"]["id"]
    payload["gears"]["headgear"]["primary_ability"] = translate_ability.get(
        int(headgear_main), ""
    )
    payload["gears"]["clothing"]["primary_ability"] = translate_ability.get(
        int(clothing_main), ""
    )
    payload["gears"]["shoes"]["primary_ability"] = translate_ability.get(
        int(shoes_main), ""
    )
    for j in range(3):
        payload["gears"]["headgear"]["secondary_abilities"].append(
            translate_ability.get(int(headgear_subs[j]), "")
        )
        payload["gears"]["clothing"]["secondary_abilities"].append(
            translate_ability.get(int(clothing_subs[j]), "")
        )
        payload["gears"]["shoes"]["secondary_abilities"].append(
            translate_ability.get(int(shoes_subs[j]), "")
        )
    ...
    # to "main process" section of monitor_battles, too. and the docs/wiki page of course
        ...
        if m_flag != -1:  # monitoring mode
            ...
            print(
                "Battle #{}: skipping upload based on ignore_private key.".format(i + 1)
            )
    else:
        # POST to stat.ink
        ...
        if payload["agent"] != os.path.basename(__file__)[:-3]:
            print(
                "Could not upload. Please contact @frozenpandaman on Twitter/GitHub for assistance."
            )
            sys.exit(1)
        postbattle = requests.post(
            url, headers=auth, data=msgpack.packb(payload), allow_redirects=False
        )
        ...
        if headerloc != None:
            ...
            if t_flag:
                ...
                if not ismonitor and len(results) > 1:
                    print(
                        "Error uploading battle #{}. Message from server:".format(i + 1)
                    )
                else:  # monitoring mode
    ...
    if salmon:  # salmon run mode
        ...
        if m_value != -1:  # m flag exists
            ...
            for i in reversed(range(n)):
                post_battle(
                    i, results, is_s, is_t, m_value, True if i == 0 else False, debug
                )
            if debug:
                print("")
